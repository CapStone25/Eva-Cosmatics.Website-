      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {blogPosts.map((post, index) => (
            <div key={index} className="group bg-card rounded-2xl overflow-hidden shadow-card hover:shadow-hover transition-all duration-500 animate-fade-in" style={{ animationDelay: `${index * 0.1}s`, animationFillMode: 'forwards' }}>
              ...
              <div className="p-6">
                ...
                <Button variant={post.featured ? "default" : "outline"} className="w-full">{t("readMore")}</Button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8 items-center bg-gradient-to-r from-secondary/40 to-secondary/20 rounded-3xl p-8 md:p-12 mt-12 animate-fade-in" style={{ animationDelay: '0.3s' }}>
          ...
          <div className="text-center md:text-start">
            ...
            <Button size="lg" className="px-12">{t("exploreMore")}</Button>
          </div>
        </div>
      </div>
    </section>
